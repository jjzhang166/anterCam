	...
	m_hCursorMove(::LoadCursor(NULL, IDC_SIZEALL))
{
	...
	m_nSrcWidth = m_rScreen.right - m_rScreen.left;  // add by dongminyi
	m_nSrcHeight = m_rScreen.bottom - m_rScreen.top;

	m_nCaptureWidth = m_nSrcWidth;//1280;//m_rScreen.right - m_rScreen.left;    // dongmingyi
	m_nCaptureHeight = m_nSrcHeight;//720;//m_rScreen.bottom - m_rScreen.top; 

	m_bIncludeDefaultSize  = false;
	int size =  sizeof(video_desc)/sizeof(MediaDescriptor);
	for( int i = 0; i < size; i++)
	{
		if( m_nSrcWidth == video_desc[i].width && m_nSrcHeight == video_desc[i].height)
		{
			m_bIncludeDefaultSize  = true;
			break;
		}
	}
...
{
	//m_nCaptureWidth = m_rScreen.right - m_rScreen.left;    // dongmingyi
...
HRESULT CCamStudioPin::FillBuffer(IMediaSample *pSample)
{
	...
	int a = pSample->GetActualDataLength();
	return S_OK;
}
...
HRESULT CCamStudioPin::SetMediaType(const CMediaType *pMediaType)
{
	...
	if(SUCCEEDED(hr))
	{
		VIDEOINFO *pvi = (VIDEOINFO *) m_mt.Format();   // modify by dongmingyi
		if (pvi == NULL)
			return E_UNEXPECTED;
		...
		*/
		case 32:    // RGB32
			...
			//	m_mt = *pMediaType;    // add by dongmingyi
			hr = S_OK;
			break;
		...
		{
			if( pvi->AvgTimePerFrame >= 1666666  )   // add by dongmingyi  帧率大于6帧不生效，防止cpu过高 8 frame rate per second
		...
		{
		m_rtFrameLength =  UNITS / 10;
		}
		*/
		//m_rScreen.right = m_rScreen.left + pvi->bmiHeader.biWidth;    // del by dongmingyi
      ...
      /**/
		if( m_nCaptureWidth != pvi->bmiHeader.biWidth && m_nCaptureHeight != pvi->bmiHeader.biHeight )
		{

			  wsprintf(errbuf, L"!!!! if m_nCaptureWidth = %d,m_nCaptureHeight=%d, w=%d,h=%d \n",m_nCaptureWidth,m_nCaptureHeight,pvi->bmiHeader.biWidth,pvi->bmiHeader.biHeight );
	         OutputDebugString(errbuf); 
			hr = E_INVALIDARG;
		}else{

			  wsprintf(errbuf, L"!!!! else m_nCaptureWidth = %d,m_nCaptureHeight=%d, w=%d,h=%d \n",m_nCaptureWidth,m_nCaptureHeight,pvi->bmiHeader.biWidth,pvi->bmiHeader.biHeight );
	    OutputDebugString(errbuf); 
		}
...
STDMETHODIMP CCamStudioPin::SetWidth(int iWidth)
{
	return S_OK;
}
//**************************************************************
...
void CCamStudioPin::CreateSample()
{
	if(m_hRawBitmap)
		DeleteObject (m_hRawBitmap);
	m_hRawBitmap = CreateCompatibleBitmap(m_hScrDc,   m_nCaptureWidth, m_nCaptureHeight);  // dongmingyi
}
...
{ 
	...
	if(hMemDC && m_hScrDc)
	{
		if( (m_nCaptureWidth == m_nSrcWidth) && (m_nCaptureHeight == m_nSrcHeight))
		{
			BitBlt(hMemDC, 0, 0, m_nCaptureWidth, m_nCaptureHeight, m_hScrDc, m_rScreen.left, m_rScreen.top, SRCCOPY);
			AddMouse(hMemDC, &m_rScreen, m_hScrDc, m_hWndToTrack);   // add
		}
		else{
			m_fScaleX = (float)( (float)m_nCaptureWidth  / (float)(m_rScreen.right-m_rScreen.left));
			m_fScaleY = (float)((float) m_nCaptureHeight / (float)(m_rScreen.bottom-m_rScreen.top));
			SetStretchBltMode(hMemDC, HALFTONE/*BLACKONWHITE*/);  
			BOOL ret = StretchBlt(hMemDC,0,0,m_nCaptureWidth,m_nCaptureHeight,m_hScrDc,m_rScreen.left,m_rScreen.top,(m_rScreen.right - m_rScreen.left),(m_rScreen.bottom - m_rScreen.top),SRCCOPY);   // dongmingyi
			DrawMouse(hMemDC,m_fScaleX, m_fScaleY ,  m_xHotspot, m_yHotspot,&m_rScreen);
		}

	}

	//AddMouse(hMemDC, &m_rScreen, m_hScrDc, m_hWndToTrack);  //del
...
void CCamStudioPin::InitIcon()
{
	...
	ICONINFO iconinfo;
	ret = GetIconInfo(cursorinfo.hCursor, &iconinfo); 
	if( ret == 0)
		return;
	m_xHotspot = iconinfo.xHotspot;
	m_yHotspot = iconinfo.yHotspot;
	if (iconinfo.hbmMask)
	{
...
STDMETHODIMP CCamStudioPin::NonDelegatingQueryInterface(REFIID riid, 
...
HRESULT CCamStudioPin::GetPages(  __RPC__out CAUUID *pPages)
{
...
//**************************************************************
//* IScreenCaptureSize implementation ends here
//**************************************************************
STDMETHODIMP CCamStudioPin::SetDstWidth(UINT iDstWidth)
{
	m_nCaptureWidth = iDstWidth; 
	return S_OK;
}
STDMETHODIMP CCamStudioPin::SetDstHeight(UINT iDstHeight)
{
	m_nCaptureHeight = iDstHeight;
	return S_OK;
}

UINT CCamStudioPin::GetDstWidth (void)
{
	return m_nCaptureWidth;
}

UINT CCamStudioPin::GetDstHeight (void)
{
	return m_nCaptureHeight;
}

UINT CCamStudioPin::GetSrcWidth (void)
{
	return m_nSrcWidth;
}

UINT CCamStudioPin::GetSrcHeight (void)
{
	return m_nSrcHeight;
}

bool  CCamStudioPin::IsIncludeDefaultSize(void)
{
	return m_bIncludeDefaultSize ;
} 
STDMETHODIMP  CCamStudioPin::SetChange(bool bChange)
{
	m_bIsChange = bChange;
	return S_OK  ;
}
...
STDMETHODIMP  CCamStudioPin::SetFrameRate (UINT iFrameRate) 
{ 
	return SetFPS( iFrameRate )  ;
}
UINT CCamStudioPin::GetFrameRate(void)
{
	return m_nFPS  ;
}